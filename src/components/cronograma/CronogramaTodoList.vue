      ...
      >
        <template #item="{ element: todo }">
          <div class="todo-item-wrapper">
            <TodoItem
              :todo="todo"
              :update-service="cronogramaUpdateService"
              @edit="editTodo"
              @delete="deleteTodo"
              @update="updateTodo"
            />
...
// Serviço wrapper para o TodoItem usar o cronogramaService
const cronogramaUpdateService = {
  async update(todoId, data) {
    return await cronogramaService.updateTodo(props.cronogramaId, todoId, data);
  }
};

const loadTodos = async () => {
  try {
    const loadedTodos = await cronogramaService.getTodos(props.cronogramaId);
...
watch(() => props.cronogramaId, (newId) => {
  if (newId) {
    loadTodos();
  }
}, { immediate: true });
...
const addTodo = async () => {
  ...
  try {
    await cronogramaService.createTodo(props.cronogramaId, { descricao: newTodoDescricao.value.trim() });
    newTodoDescricao.value = '';
    // WebSocket vai atualizar automaticamente via evento 'cronograma-todo-created'
  } catch (error) {
...
const saveTodo = async () => {
  ...
  try {
    await cronogramaService.updateTodo(props.cronogramaId, editingTodo.value.id, form.value);
    toast.success('Todo atualizado com sucesso!');
    closeModal();
    // WebSocket vai atualizar automaticamente via evento 'cronograma-todo-updated'
  } catch (error) {
...
const deleteTodo = async (id) => {
  ...
  try {
    await cronogramaService.deleteTodo(props.cronogramaId, id);
    toast.success('Todo excluído com sucesso!');
    // WebSocket vai atualizar automaticamente via evento 'cronograma-todo-deleted'
  } catch (error) {
...
const updateTodo = (updatedTodo) => {
  ...
  if (index !== -1) {
    ...
    // Não emitir todos-updated aqui para evitar refresh desnecessário
    // O WebSocket vai atualizar automaticamente via evento 'cronograma-todo-updated'
  }
};
...
const marcarTodosConcluidos = async () => {
  ...
  try {
    const promises = pendentes.map(todo =>
      cronogramaService.updateTodo(props.cronogramaId, todo.id, { concluido: true })
    );
    await Promise.all(promises);
    toast.success('Todos marcados como concluídos!');
    // WebSocket vai atualizar automaticamente via eventos 'cronograma-todo-updated'
  } catch (error) {
...
const desmarcarTodosConcluidos = async () => {
  ...
  try {
    const promises = concluidos.map(todo =>
      cronogramaService.updateTodo(props.cronogramaId, todo.id, { concluido: false })
    );
    await Promise.all(promises);
...
const onDragEnd = async () => {
  try {
    ...
    await cronogramaService.reorderTodos(props.cronogramaId, todosOrdenados);
    toast.success('Ordem atualizada!');
    // WebSocket vai atualizar automaticamente via evento 'cronograma-todos-reordered'
  } catch (error) {
